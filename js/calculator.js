...
const updatePower = () => {
    ...
    if (calculated_power.value > 2.250 && calculated_power.value < 3.750) {
    ...
    document.getElementById('marzec-wartosc').innerText = parseFloat(srednia_produkcja * 0.0810).toFixed(2) + ' kWh';
    ...
    document.getElementById('listopad-wartosc').innerText = parseFloat(srednia_produkcja * 0.0290).toFixed(2) + ' kWh';
...
const calculatePrice = (el) => {
    ...
    if (calculated_power.value < 3 && document.getElementById('have_pv_installation').value === "Nie") {
    ...
    if (document.getElementById("trina-solar").selected == true ||
        document.getElementById("trina-solar2").selected == true) {
...
const updatePrice = (el) => {
    ...
    const large_inverter = document.getElementById('large_inverter'); let custom_cost = 0;
    if (document.getElementById('have_pv_installation').value === 'Tak') {
        custom_cost += 1500;
    }
...
const productionUpdate = () => {
    ...
    for (let year = 1; year <= years; year++) {
        ...
        res += `<div class="td ${savedSum - finalPrice < 0 ? 'price-less' : savedSum - finalPrice > 0 ? 'price-more' : 'neutral'}">${Intl.NumberFormat('pl-PL').format(parseFloat(savedSum -
            finalPrice).toFixed(2))}zł</div>`;
        res += '</div>';
    ...
    document.getElementById('saved-money').innerHTML = `${Intl.NumberFormat('pl-PL').format(parseFloat(savedSum -
